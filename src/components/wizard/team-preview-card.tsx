...
function Escudo({ url }: { url?: string | null }) {
  ...
  return (
    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-muneca-purple/20 text-muneca-yellow">
...
export function TeamPreviewCard({ team }: { team: TeamPreviewData }) {
  ...
  return (
    <div className="hidden xl:block">
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate font-display text-lg uppercase tracking-wide text-white">
              {tieneNombre ? team.nombreEquipo : "Tu equipo"}
            </p>
            <span className="inline-block rounded-full bg-muneca-purple/20 px-2.5 py-0.5 text-xs font-semibold text-muneca-yellow">
              Categoría Libre
            </span>
          </div>
        </div>
          ...
          <dl className="mt-4 space-y-2 border-t border-white/10 pt-4 text-sm">
            {team.delegadoNombre && (
              <div className="flex items-center gap-2 text-white/70">
                <span className="text-white/40">Delegado</span>
                <span className="font-medium text-white">{team.delegadoNombre}</span>
              </div>
            )}
            {team.delegadoContacto && (
              <div className="flex items-center gap-2 text-white/70">
                <span className="text-white/40">Contacto</span>
                <span className="font-medium text-white">{team.delegadoContacto}</span>
              </div>
            )}
            {team.delegadoDocumento && (
              <div className="flex items-center gap-2 text-white/70">
                <span className="text-white/40">Documento</span>
                <span className="font-medium text-white">{team.delegadoDocumento}</span>
              </div>
            )}
          </dl>
        )}
      ...
      <div className="relative mt-5 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        <div
          aria-hidden="true"
